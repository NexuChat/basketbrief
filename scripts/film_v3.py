...
# Introduce the product and participants before the chronological demonstration.
SCENES=[
 {'id': 'cold-open', 'kind': 'motion', 'chapter': 'BASKETBRIEF', 'title': 'Relief work.\nThen paperwork.', 'sub': 'Follow-up for small relief teams.', 'voice': 'A relief delivery is only part of the job. The coordinator still needs the receipts, the counts, and a report for every donor.', 'hold': 9},
 {'id': 'introduction', 'kind': 'screen', 'asset': 'overview', 'chapter': 'MEET BASKETBRIEF', 'title': 'Bring the evidence\ntogether.', 'sub': 'Follow up with contributors.\nPrepare reports for human review.', 'voice': 'BasketBrief helps small relief teams turn scattered evidence into approved donor reports. Here is one complete example.', 'hold': 9},
 {'id': 'people', 'kind': 'motion', 'chapter': 'OUR DEMONSTRATION TEAM', 'title': 'One coordinator.\nA scattered paper trail.', 'sub': 'Rana has the receipts. Sami has the counts.\nAmal brings the evidence to both donors.', 'voice': 'Meet Amal, our fictional coordinator. Rana has the receipts. Sami tracks deliveries. Amal needs a consistent report for two donors.', 'hold': 9},
 {'id': 'promise', 'kind': 'motion', 'chapter': 'THE WORKFLOW', 'title': 'Follow up.\nReview. Share.', 'sub': 'From missing evidence to an approved report.', 'voice': 'The agent asks contributors for missing evidence and prepares the report. Amal reviews it and decides what gets shared. Let us follow that process.', 'hold': 9},
 ...
 {'id': 'turn', 'kind': 'motion', 'chapter': 'LATER IN THIS EXAMPLE', 'title': 'A new update\nfrom Sami.', 'sub': 'Delivered kits: 92 → 88', 'voice': 'After these reports are sent, Sami updates the delivered count from ninety-two kits to eighty-eight. The team now needs an amended report.', 'hold': 9},
...
def page_for(s,i):
    ...
    if s['kind']=='screen':
        ...
        extra='<div class="message reveal"><small>THE COORDINATOR’S INBOX</small><b>Receipts. Updates. Donor requests.</b></div>'
    elif s['id']=='people':
        extra='<div class="diagram"><div class="node enter"><b>Rana + Sami</b><span>Receipts + delivery counts</span></div><span class="arrow">→</span><div class="node dark enter"><b>Amal</b><span>The coordinator</span></div><span class="arrow">→</span><div class="node enter"><b>Two donors</b><span>One consistent account</span></div></div>'
    elif s['id']=='promise':
    ...
    elif s['id']=='turn':extra='<div class="numbers"><span class="old">92</span><span class="new reveal">88</span></div>'
...
def caption_and_mix():
    ...
    final=OUT/'BasketBrief-polished-20260914.mp4';run('ffmpeg','-y','-v','error','-i',str(silent),'-i',str(track),'-map','0:v','-map','1:a','-c','copy','-movflags','+faststart','-shortest',str(final))
    (OUT/'BasketBrief-polished-20260914.en.vtt').write_text('WEBVTT\n\n'+''.join(f'{ts(a)} --> {ts(b)}\n{c}\n\n' for a,b,c in cues))
    (OUT/'BasketBrief-polished-20260914.en.srt').write_text(''.join(f'{i}\n{ts(a,",")} --> {ts(b,",")}\n{c}\n\n' for i,(a,b,c) in enumerate(cues,1)))
    for extension in ('en.vtt','en.srt'):
        path=OUT/f'BasketBrief-polished-20260914.{extension}'
