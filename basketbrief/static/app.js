...
async function showSource(id) {const e=state.evidence.find(e=>e.id===Number(id));if(!e)return;$('#source-content').innerHTML=`<h2>${esc(title(e))}</h2><p class="form-hint">${esc(names[e.actor])} · Source ${e.id} · ${esc(e.note||e.status)}</p><pre>${esc(e.text)}</pre>`;$('#source-dialog').showModal();if(e.attachment){try{const r=await api('/evidence/'+e.id+'/image',{raw:true});const url=URL.createObjectURL(await r.blob());const img=document.createElement('img');img.alt='Original uploaded receipt';img.src=url;img.onload=()=>URL.revokeObjectURL(url);$('#source-content').append(img);}catch(err){toast(err.message);}}}
